// Quick script to check companies in Firestore
    ...
    return new Promise((resolve, reject) => {
        ...
        const req = https.request(options, (res) => {
            ...
            res.on('end', () => {
                try {
                    const response = JSON.parse(body);
                    console.log('\n📋 Companies collection query result:');
                    ...
                    if (response.documents) {
                        ...
                        response.documents.forEach((doc, index) => {
                            ...
                            const tier = doc.fields?.tier?.stringValue || 'No tier';
                            const email = doc.fields?.adminEmail?.stringValue || 'No email';
                        ...
                        console.log('   ❌ No companies found in collection');
                        if (response.error) {
...
async function main() {
    try {
        ...
        console.log('\n════════════════════════════════════════════');
        console.log('🔍 COMPANIES CHECK COMPLETE');
        console.log('════════════════════════════════════════════');
