...
const AdminDashboard: React.FC = () => {
  ...
  const fetchCompanies = async () => {
    try {
      console.log('[ADMIN DASHBOARD] Starting to fetch companies...');
      setIsLoading(true);
      const data = await getCompanies();
      console.log('[ADMIN DASHBOARD] Received companies data:', data);
      console.log('[ADMIN DASHBOARD] Number of companies:', data.length);
      setCompanies(data);
      setIsLoading(false);
    } catch (error) {
      console.error('[ADMIN DASHBOARD] Error fetching companies:', error);
      setIsLoading(false);
      toast.error('Gagal memuat data perusahaan: ' + (error as Error).message);
    }
  };
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewCompany({ name: '', email: '', tier: 'Freemium' });
      ...
      <div className="bg-white dark:bg-brand-slate-850 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm p-2">
        <div className="flex gap-2">
          <button
            ...
            className={`flex-1 py-3 px-6 rounded-xl font-semibold transition-all flex items-center justify-center gap-2 ${activeTab === 'management'
          ...
          <button
            ...
            className={`flex-1 py-3 px-6 rounded-xl font-semibold transition-all flex items-center justify-center gap-2 ${activeTab === 'analytics'
              ? 'bg-brand-orange text-white shadow-md'
              : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-800'
              }`}
          >
                ...
                <form onSubmit={handleInvite} className="p-6 space-y-4">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {/* Freemium Tier */}
                      <label className={`p-3 rounded-xl border cursor-pointer flex flex-col items-center justify-center gap-1 text-xs text-center transition-all ${newCompany.tier === 'Freemium' ? 'border-gray-400 bg-gray-100 text-gray-800 font-bold shadow-sm ring-1 ring-gray-300' : 'border-gray-200 dark:border-slate-600 text-gray-500 hover:bg-gray-50 dark:hover:bg-slate-800'}`}>
                        <input type="radio" name="tier" value="Freemium" className="hidden" checked={newCompany.tier === 'Freemium'} onChange={() => setNewCompany({ ...newCompany, tier: 'Freemium' })} />
                        <span className="font-bold">Freemium</span>
                        <span className="text-[10px] opacity-70">Starter (1000 credits)</span>
                      </label>
                      ...
                      <label className={`p-3 rounded-xl border cursor-pointer flex flex-col items-center justify-center gap-1 text-xs text-center transition-all ${newCompany.tier === 'Premium' ? 'border-brand-blue bg-blue-50 text-brand-blue font-bold shadow-sm ring-1 ring-brand-blue' : 'border-gray-200 dark:border-slate-600 text-gray-500 hover:bg-gray-50 dark:hover:bg-slate-800'}`}>
                        ...
                        <span className="text-[10px] opacity-70">Pro (1500 credits)</span>
                      </label>
                    </div>
                  </div>
