...
/**
 * Fungsi: generateNextQuestionDynamic
 * Trigger: Frontend (PublicAssessment - during interview)
 * Deskripsi: Generate pertanyaan follow-up dinamis berdasarkan jawaban kandidat
 * Region: europe-west1
 */
exports.generateNextQuestionDynamic = onCall({ region: "europe-west1" }, async (request) => {
  const { GoogleGenerativeAI, HarmCategory, HarmBlockThreshold } = require("@google/generative-ai");
  const functions = require("firebase-functions");
  ...
  const { role, history, assessmentData } = request.data;

  if (!role || !history) {
    throw new HttpsError('invalid-argument', 'Parameter role dan history wajib diisi.');
  }

  const questionCount = history.filter(h => h.speaker === 'ai').length;

  // Check if we should end the interview (after ~8 questions)
  if (questionCount >= 8) {
    return {
      success: true,
      response: "Terima kasih atas semua jawaban Anda. Sesi wawancara telah selesai. Kami akan segera memproses hasil assessment Anda."
    };
  }

  const context = history.slice(-6).map(h => `${h.speaker.toUpperCase()}: ${h.text}`).join('\n');
  const lastUserMessage = history.length > 0 ? history[history.length - 1].text : "";

  // Build context from assessment data if available
  let assessmentContext = "";
  if (assessmentData && assessmentData.structuredAssessment) {
    const highRiskAnswers = assessmentData.structuredAssessment
      .filter(item => item.response >= 4)
      .map(item => `${item.category}: ${item.question}`)
      .slice(0, 3);

    if (highRiskAnswers.length > 0) {
      assessmentContext = `\nAREAS PERLU DIGALI LEBIH DALAM:\n${highRiskAnswers.join('\n')}`;
    }
  }

  const prompt = `Anda adalah AI Interviewer profesional untuk posisi ${role} yang sedang melakukan fraud risk assessment interview.

KONTEKS PERCAKAPAN (6 chat terakhir):
${context}
...
INSTRUKSI:
1. Buat follow-up question yang:
   - PROBING dan menggali lebih dalam tentang integritas kandidat
   - NATURAL dan conversational dalam Bahasa Indonesia
   - SPESIFIK terkait jawaban kandidat atau area risk yang teridentifikasi
   - Fokus pada mengungkap red flags: tekanan finansial, rasionalisasi fraud, atau kesempatan

2. Pertanyaan harus mengeksplorasi:
   - Bagaimana kandidat menghadapi tekanan atau dilema etika
   - Pengalaman dengan situasi konflik kepentingan
   - Cara kandidat memandang aturan dan compliance
   - Pola perilaku dalam menghadapi opportunity untuk fraud

3. PENTING: Response Anda HANYA berisi 1 pertanyaan follow-up, tidak ada embel-embel lain.

CONTOH PERTANYAAN YANG BAIK:
- "Anda menyebutkan pernah mengalami tekanan target. Bisa ceritakan lebih detail bagaimana Anda menanganinya tanpa melanggar prosedur?"
- "Menarik bahwa Anda pernah menemukan kesalahan laporan. Apa yang membuat Anda memutuskan untuk melaporkannya?"
- "Bagaimana Anda menangani situasi ketika atasan meminta Anda melakukan sesuatu yang Anda rasa tidak tepat?"

Sekarang, generate 1 pertanyaan follow-up yang probing dan contextual:`;

  // Try Gemini first
  if (GEMINI_API_KEY) {
    try {
      console.log(`[QUESTION] Generating dynamic question for ${role} (question #${questionCount + 1})`);
      ...
      // Clean up response
      aiResponse = aiResponse.replace(/^AI:/i, "").replace(/^Interviewer:/i, "").replace(/^Question:/i, "").trim();
      // Remove quotes if wrapped
      aiResponse = aiResponse.replace(/^["']/, "").replace(/["']$/, "");

      console.log(`[QUESTION] Gemini generated: ${aiResponse.substring(0, 50)}...`);
      ...
      console.warn("[WARN] Gemini failed for question generation, trying OpenAI:", geminiError.message);
    }
  }
  ...
  if (OPENAI_API_KEY) {
    try {
      console.log(`[QUESTION] Trying OpenAI fallback`);
      ...
      const response = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
          model: "gpt-3.5-turbo",
          messages: [
            { role: "system", content: "Anda adalah HR Interviewer profesional yang melakukan fraud risk assessment dalam Bahasa Indonesia." },
            { role: "user", content: prompt }
          ],
          max_tokens: 150,
          temperature: 0.8
      ...
      if (data.choices && data.choices[0] && data.choices[0].message) {
        ...
        console.log(`[QUESTION] OpenAI generated: ${aiResponse.substring(0, 50)}...`);
  ...
  // Final fallback - contextual static questions
  console.log("[QUESTION] All AI providers failed, using contextual fallback");

  const fallbackQuestions = [
    "Bisa Anda ceritakan lebih detail tentang bagaimana Anda menangani tekanan dalam pekerjaan?",
    "Ceritakan pengalaman Anda ketika menghadapi dilema etika di tempat kerja.",
    "Bagaimana Anda bereaksi jika menemukan ketidaksesuaian dalam proses atau laporan?",
    "Apa yang akan Anda lakukan jika atasan meminta Anda melakukan sesuatu yang melanggar prosedur?",
    "Ceritakan tentang situasi ketika Anda harus memilih antara target dan integritas.",
    "Bagaimana Anda membangun kepercayaan dengan tim dan atasan?",
    "Ceritakan keputusan sulit yang pernah Anda ambil terkait compliance atau etika kerja."
  ];

  return {
    success: true,
    response: fallbackQuestions[questionCount % fallbackQuestions.length]
  };
});